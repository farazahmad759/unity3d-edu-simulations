...
public class WMG_Series : MonoBehaviour {
		...
		if (areaShadingUsesComputeShader) {
			...
			if (areaShadingType != areaShadingTypes.None) {
				if (areaShadingRects.Count != 1) {
					...
					theGraph.changeSpritePivot(curObj, WMG_Text_Functions.WMGpivotTypes.BottomLeft);
	...
	void getNewPointPositionsAndSizes(List<GameObject> prevPoints, ref List<Vector2> newPositions, ref List<int> newWidths, ref List<int> newHeights) {
		...
		float yAxisMax = theGraph.yAxis.AxisMaxValue;
		float xAxisMin = theGraph.xAxis.AxisMinValue;
		float yAxisMin = theGraph.yAxis.AxisMinValue;
		...
		for (int i = 0; i < dataLabels.Count; i++) {
			...
			if (seriesIsLine) {
				...
				if (theGraph.orientationType == WMG_Axis_Graph.orientationTypes.vertical) {
					...
					if (barIsNegative[i]) {
						newY = -dataLabelsOffset.y - theGraph.getSpriteHeight(points[i]) + Mathf.RoundToInt((theGraph.barAxisValue - theGraph.yAxis.AxisMinValue) / (theGraph.yAxis.AxisMaxValue - theGraph.yAxis.AxisMinValue) * theGraph.yAxisLength);
					}
					theGraph.changeSpritePositionTo(dataLabels[i], new Vector3(
	...
	public void updateAreaShading(List<Vector2> newPositions) {
		...
		if (areaShadingUsesComputeShader && areaShadingRects.Count == 1) { // all values (min, max, and the points) represent a percentage of graph
			...
			if (theGraph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal) {
				...
				areaShadingCS.computeShader.SetFloat ("minVal", (areaShadingAxisValue - theGraph.yAxis.AxisMinValue) / (theGraph.yAxis.AxisMaxValue - theGraph.yAxis.AxisMinValue));
			}
			float maxVal = 0;
			for (int i = 0; i < pointValues.Count; i++) {
			...
			for (int i = 0; i < points.Count - 1; i++) {
				...
				float axisMultiplier = theGraph.yAxisLength / (theGraph.yAxis.AxisMaxValue - theGraph.yAxis.AxisMinValue);
				float yPosOfAxisVal = (areaShadingAxisValue - theGraph.yAxis.AxisMinValue) * axisMultiplier;
				if (theGraph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal) {
					rotation = 90;
	...
	private void DoRealTimeUpdate() {
		...
		if (pointValues.Count >= 2) {
			...
			float graphSlope = (theGraph.yAxis.AxisMaxValue - theGraph.yAxis.AxisMinValue) / (theGraph.xAxis.AxisMaxValue - theGraph.xAxis.AxisMinValue);
